...
var q = async.queue(function(task, callback){
  var form = task.form.replace(/%month/g, task.month).replace(/%day/g, task.day).replace(/%year/g, task.year);
  var name = [ task.month, task.day, task.year].join('.');
  var cmd = 'curl -X POST --data "'+ form +'" http://wonder.cdc.gov/controller/datarequest/D60a -o data/' + name + '.tsv';
  console.log( cmd );
...
while ( year < 2012 ){
  ...
  while ( i < 13){
    ...
    while ( j < days+1){
      ...
      q.push({year: year, month:month, day: day, form: form});
      j++;
    }
    i++;
  }
